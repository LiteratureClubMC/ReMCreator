		...
		if (element != null) {
			...
			if (element instanceof FolderElement) {
				...
				if (modIcon != null && modIcon.getImage() != null && modIcon.getIconWidth() > 0
					...
					if (dva != null) {
						ImageIcon iconbig = ImageUtils.drawOver(
								TiledImageCache.getModTypeIcon(((ModElement) element).getType()), dva);
						icon.setIcon(new ImageIcon(ImageUtils.resize(iconbig.getImage(), 25)));
					} else {
						icon.setIcon(new ImageIcon(ImageUtils.resizeAA(
								TiledImageCache.getModTypeIcon(((ModElement) element).getType()).getImage(), 25)));
					}
				}
			}
