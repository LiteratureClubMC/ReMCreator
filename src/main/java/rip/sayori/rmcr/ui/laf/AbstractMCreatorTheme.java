...
public abstract class AbstractMCreatorTheme extends OceanTheme {
	...
	public AbstractMCreatorTheme() {
		...
		try {
			...
			if (!NON_ROBOTO_LANGUAGES.contains(lang))
				light_font = Font.createFont(Font.TRUETYPE_FONT,
						getClass().getResourceAsStream("/fonts/roboto_light.ttf"));
	...
	@Override public void addCustomEntriesToTable(UIDefaults table) {
		...
		table.put("ToolBar.rolloverBorder",
				BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(getDarkAccent(), 1),
						BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(getLightAccent(), 1),
		...
		List<?> buttonGradient = Arrays.asList(0f, 0f, new ColorUIResource(getBrightColor()),
				new ColorUIResource(getBrightColor()), new ColorUIResource(getBrightColor()));
		...
		List<?> sliderGradient = Arrays.asList(0f, 0f, new ColorUIResource(getDarkAccent()),
				new ColorUIResource(getDarkAccent()), new ColorUIResource(getDarkAccent()));
