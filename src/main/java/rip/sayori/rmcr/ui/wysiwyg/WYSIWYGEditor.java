...
import rip.sayori.rmcr.element.parts.gui.*;
...
import rip.sayori.rmcr.ui.dialogs.wysiwyg.*;
...
public class WYSIWYGEditor extends JPanel {
	...
	public WYSIWYGEditor(final MCreator mcreator, boolean isNotOverlayType) {
		...
		if (isNotOverlayType) {
			...
			ovst.add(PanelUtils.join(FlowLayout.LEFT,
					HelpUtils.wrapWithHelpButton(IHelpContext.NONE.withEntry("overlay/rendering_priority"),
			...
			ovst.add(PanelUtils.northAndCenterElement(
					HelpUtils.wrapWithHelpButton(IHelpContext.NONE.withEntry("overlay/base_texture"),
